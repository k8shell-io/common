...
// Watcher watches a directory tree for file changes and calls onReload() debounced.
type Watcher struct {
	...
	watchEnabled bool
	extensions   []string // Allowed file extensions to watch
...
// extensions: list of file extensions to watch (e.g., []string{".yaml", ".yml", ".crt", ".key", ".pem"})
// If extensions is empty or nil, all files will be watched.
func NewWatcher(watchDir string, reloadDelay time.Duration, extensions []string, onReload func() error) *Watcher {
...
// isAllowedFile checks if a file has an allowed extension
func (w *Watcher) isAllowedFile(filename string) bool {
	...
	for _, allowedExt := range w.extensions {
