...
// rule identified by id. idp/org are immutable — delete and recreate the
// rule to change them.
type UpdateOnboardRuleRequest struct {
	...
	// username_pattern replaces the rule's matching pattern: '*', a
	// comma-delimited list of usernames, or a single exact username. Required,
	// same as on CreateOnboardRuleRequest.
	UsernamePattern string `protobuf:"bytes,9,opt,name=username_pattern,json=usernamePattern,proto3" json:"username_pattern,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}
...
func (x *UpdateOnboardRuleRequest) GetUsernamePattern() string {
	if x != nil {
		return x.UsernamePattern
	}
	return ""
...
type Announcement struct {
	...
	// email_enabled marks this announcement as eligible to be sent by email,
	// in addition to the per-user announcementsByEmail opt-in stored in
	// UserSettings — an announcement is only emailed to a user when both are
	// true. Defaults to false (not email-eligible) when omitted. No sender
	// exists yet; this only records the flag.
	EmailEnabled  bool `protobuf:"varint,15,opt,name=email_enabled,json=emailEnabled,proto3" json:"email_enabled,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *Announcement) GetEmailEnabled() bool {
	if x != nil {
...
const file_identity_v1_types_proto_rawDesc = "" +
	...
	"\x04note\x18\b \x01(\tR\x04note\"\xf9\x01\n" +
	...
	"\x10username_pattern\x18\t \x01(\tR\x0fusernamePattern\"*\n" +
	...
	"\x04body\x18\x02 \x01(\tR\x04body\"\xd3\x04\n" +
	...
	"\remail_enabled\x18\x0f \x01(\bR\femailEnabled\"S\n" +
